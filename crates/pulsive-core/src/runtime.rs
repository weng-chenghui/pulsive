...
impl Runtime {
    ...
    pub fn on_tick(&mut self, handler: TickHandler) {
        ...
        self.tick_handlers.sort_by(|a, b| b.priority.cmp(&a.priority));
    }
    ...
    pub fn tick(&mut self, model: &mut Model) -> UpdateResult {
        ...
        let due: Vec<Msg> = self.scheduled
            .iter()
        ...
        match msg.kind {
            ...
            MsgKind::Event | MsgKind::ScheduledEvent => {
                ...
                if let Some(event_id) = &msg.event_id {
                    let handlers: Vec<_> = self.event_handlers
                        .iter()
        ...
        if let Some(kind) = &handler.target_kind {
            ...
            for entity_id in entity_ids {
                ...
                for effect in &handler.effects {
                    self.execute_effect(model, effect, &target, &msg.params, &mut result.effect_result);
                }
            }
        } else {
            ...
            for effect in &handler.effects {
                self.execute_effect(model, effect, &EntityRef::Global, &msg.params, &mut result.effect_result);
            }
        }
    }
    ...
    fn run_event_handler(
        ...
        // Execute effects
        for effect in &handler.effects {
            self.execute_effect(model, effect, &msg.target, &msg.params, &mut result.effect_result);
        }
    }
    ...
    fn execute_effect(
        ...
        match effect {
            ...
            Effect::ModifyProperty { property, op, value } => {
                // Evaluate with target entity context
                let target_entity = model.entities.resolve(target);
                let mut ctx = EvalContext::new(
                    &model.entities,
            ...
            Effect::ModifyGlobal { property, op, value } => {
                let mut ctx = EvalContext::new(
                    &model.entities,
                    &model.globals,
                    params,
                ...
                if let Ok(v) = value.eval(&mut ctx) {
                    if let Some(operand) = v.as_float() {
                        let current = model.globals
                            .get(property)
                            .and_then(|v| v.as_float())
                            .unwrap_or(0.0);
                        let new_value = op.apply(current, operand);
                        model.globals.insert(property.clone(), Value::Float(new_value));
                    }
                }
            }
            Effect::AddFlag(flag) => {
            ...
            Effect::EmitEvent { event, target: event_target, params: event_params } => {
                ...
                result.emitted_events.push((event.clone(), event_target.clone(), evaluated_params));
            }
            Effect::ScheduleEvent { event, target: event_target, delay_ticks, params: event_params } => {
                let mut ctx = EvalContext::new(
                    &model.entities,
                    &model.globals,
                    params,
            ...
            Effect::If { condition, then_effects, else_effects } => {
                let mut ctx = EvalContext::new(
                    &model.entities,
                    &model.globals,
                    params,
            ...
            Effect::ForEachEntity { kind, filter, effects } => {
                let entity_ids: Vec<_> = model.entities.by_kind(kind).map(|e| e.id).collect();
            ...
            Effect::RandomChoice { choices } => {
                ...
                for (weight_expr, _) in choices {
                    ...
                    let weight = weight_expr.eval(&mut ctx)
                        .ok()
                        .and_then(|v| v.as_float())
                        .unwrap_or(0.0);
                    weights.push(weight);
            ...
            Effect::Notify { kind, title, message, target: notify_target } => {
                let mut ctx = EvalContext::new(
                    &model.entities,
                    &model.globals,
                    params,
                ...
                let title_str = title.eval(&mut ctx)
                    .map(|v| format!("{}", v))
                    .unwrap_or_default();
                let msg_str = message.eval(&mut ctx)
                    .map(|v| format!("{}", v))
                    .unwrap_or_default();
...
mod tests {
    ...
    fn test_runtime_tick() {
        ...
        assert_eq!(model.get_global("tick_count").and_then(|v| v.as_float()), Some(3.0));
    ...
    fn test_runtime_event() {
        ...
        // Send event
        let msg = Msg::event("add_gold", EntityRef::Entity(entity_id), 0)
